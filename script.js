...
const getNextStoryResponse = async (data) => {
  data = "Write me a story about" + data + ".";
        const response = await fetch(
            ...
            {
                headers: {
                     Authorization:  "Bearer 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",
...
function createContinue(){
    continueButton.addEventListener('click', async () =>{
        let continuestory = await getNextStoryResponse(storyContext);
        storyContext = continuestory;
        const story = document.createElement("p");
        story.classList.add("story-text");
        story.innerHTML = continuestory;
        responseContainer.appendChild(story);
